...
const reducer = (origin = initialState, walletAction: WalletAction) => {
  return produce(origin, state => {
    switch (walletAction.type) {
      ...
      case 'deleteAccount':
        if (walletAction.payload.type == CHAINS.eth) {
          const index = state.ethaccountList.findIndex((x, i) => x.address === walletAction.payload.address)
          if (index !== -1) {
            state.ethaccountList.splice(index, 1)
          }
        } else if (walletAction.payload.type == CHAINS.bnb) {
          const index = state.bscaccountList.findIndex((x, i) => x.address === walletAction.payload.address)
          if (index !== -1) {
            state.bscaccountList.splice(index, 1)
          }
        } else if (walletAction.payload.type == CHAINS.ht) {
          const index = state.htaccountList.findIndex((x, i) => x.address === walletAction.payload.address)
          if (index !== -1) {
            state.htaccountList.splice(index, 1)
          }
        }
        return;
      case 'createUser':
